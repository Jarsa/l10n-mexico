...
import odoo.addons.l10n_mx_toponym as toponym_pkg
...
SEPOMEX_STATE_TO_SAT = {
    ...
    "06": "COL", "07": "CHP", "08": "CHH", "09": "DF", "10": "DUR",
...
CSV_PATH = join(dirname(realpath(toponym_pkg.__file__)), "static/data", "res.city.zip.csv")
txt_path = expanduser(os.environ["SEPOMEX_TXT"])
print("SEPOMEX source: %s" % txt_path)

# --- Additively load the SEPOMEX colonies into the database ----------------
env.cr.execute(
...
with open(txt_path, encoding="latin-1") as fh:
    ...
    for row in reader:
        ...
        if not city_id:
            ...
            city_id = state_mnpio_to_city.get((sat_state, (row["c_mnpio"] or "").strip()))
        if not city_id:
            skipped += 1
            continue
...
if to_create:
    ...
    env.cr.commit()
print("Inserted %d new colonies (skipped %d without a resolvable city)." % (len(to_create), skipped))

# --- Dump the full table back into the module CSV --------------------------
env.cr.execute(
    """
    SELECT
    ...
    FROM res_city_zip z
    LEFT JOIN LATERAL (
        SELECT name FROM ir_model_data WHERE model = 'res.city' AND res_id = z.city_id LIMIT 1
    ) cd ON TRUE
    LEFT JOIN LATERAL (
        SELECT name FROM ir_model_data
        WHERE model = 'l10n_mx_edi.res.locality' AND res_id = z.l10n_mx_edi_locality_id LIMIT 1
    ) ld ON TRUE
